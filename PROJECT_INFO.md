...
### Приоритетные
- [ ] **Имя из username** — в AddContact при декодировании QR без поля `name` контакт сохраняется как «Новый контакт»; при ручном вводе по @username поле «Имя» пустое, но username предзаполнен из URL-параметра — это корректно, однако нужно подтвердить поведение
- [ ] **Онбординг дважды** — `needsTour` в App.tsx инициализируется из `!localStorage.getItem("w52_tour_completed")`. У нового пользователя оба ключа отсутствуют → `needsSetup=true` и `needsTour=true` одновременно. После Setup Tour отображается корректно один раз; но если `w52_profile` удалён вручную при сохранённом `w52_tour_completed` — вёдет себя нестандартно
- [ ] **Публичный профиль** — при открытии `/u/username` без `d=` параметра ищет в localStorage-контактах; если не находит → «нет в Echo» (даже если это собственный профиль с другим `username` в поле)
- [ ] **Старый UI PublicProfile** — `PublicProfile.tsx` использует iOS light-стили (#007AFF, белый фон) — не обновлён под Echo oklch dark тему

### Средний приоритет
- [ ] **Засечки в мелких текстах** — `inputStyle` в Setup.tsx имеет `fontFamily: "var(--serif)"` → все поля ввода на шагах Setup рендерятся с засечками; нужно заменить на `var(--sans)`
- [ ] **Тег «TG» в Tasks** — в Tasks.tsx (строки 197–222) кнопка открывает `https://t.me/@username`; выглядит как тег, путает пользователей; нужно убрать
- [ ] **Setup: НИШИ / PRESET_TAGS** — в Setup.tsx лейбл «НИШИ» и массив `PRESET_TAGS` (10 старых значений) не синхронизированы с `TAG_GROUPS`; нужно заменить на `TAG_GROUPS` с группами и переименовать лейбл в «ТЕГИ»
- [ ] **Setup: заголовок** — строка 151: «Ваша визитка» → «Ваш профиль»
