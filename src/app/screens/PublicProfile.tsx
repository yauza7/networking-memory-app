...
import { mockContacts, currentUser, type User } from "../utils/mockData";
import { MessageCircle, UserPlus, Building2, Globe, Mail, Phone, ExternalLink, ArrowLeft, Sparkles } from "lucide-react";
import { motion } from "motion/react";

function findUserByUsername(username: string): User | null {
  if (currentUser.username === username) return currentUser;
  return mockContacts.find((c) => c.user.username === username)?.user ?? null;
}
...
export function PublicProfile() {
  ...
  const handleSaveContact = () => navigate(`/onboarding?saveContact=${username}`);

  const handleShare = () => {
    const url = `https://netcard.app/u/${username}`;
  ...
  return (
    <div className="min-h-screen pb-8">
      ...
      >
        ...
        <p style={{ fontSize: "15px", color: "#8E8E93", marginTop: "4px", textAlign: "center" }}>{user.role}</p>
        {user.company && (
      ...
      <div className="px-4 space-y-3">
        ...
        >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full rounded-[14px] font-semibold transition-all active:scale-97"
            style={{
              background: "rgba(255,255,255,0.72)",
              border: "0.5px solid rgba(0,0,0,0.1)",
              color: "#007AFF",
        ...
        <p style={{ textAlign: "center", color: "#C7C7CC", fontSize: "12px", paddingTop: "4px" }}>
          Профиль создан в W·52 · netcard.app
        </p>
      </div>
    </div>
  );
