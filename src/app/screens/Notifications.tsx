...
interface Notification {
  id: string;
  type: "contact_added" | "message" | "reminder";
  title: string;
  message: string;
  timestamp: string;
  read: boolean;
...
const ALL_NOTIFICATIONS: Notification[] = [
  {
    id: "1", type: "contact_added", title: "Новый контакт",
    message: "Иван Петров добавил вас в свои контакты",
    timestamp: "2026-05-14T10:30:00Z", read: false, contactName: "Иван Петров",
  },
  {
    id: "2", type: "reminder", title: "Напоминание о follow-up",
    message: "Не забудьте написать Марии Соколовой",
    timestamp: "2026-05-14T09:00:00Z", read: false, contactName: "Мария Соколова",
  },
  {
    id: "3", type: "message", title: "Новое сообщение",
    message: "Дмитрий Волков прислал вам сообщение в Telegram",
    timestamp: "2026-05-13T18:45:00Z", read: true, contactName: "Дмитрий Волков",
  },
];

const STORAGE_KEY = "w52_notifications_read";
...
export function Notifications() {
  ...
  // Init with persisted read state
  const [readIds, setReadIds] = useState<Set<string>>(() => {
    const persisted = getReadIds();
    // also mark anything that was already read in source data
    ALL_NOTIFICATIONS.filter((n) => n.read).forEach((n) => persisted.add(n.id));
    return persisted;
  });

  const notifications = ALL_NOTIFICATIONS.map((n) => ({ ...n, read: readIds.has(n.id) }));
  ...
  const markAllAsRead = useCallback(() => {
    setReadIds((prev) => {
      const next = new Set(prev);
      ALL_NOTIFICATIONS.forEach((n) => next.add(n.id));
      saveReadIds(next);
      return next;
    });
  }, []);
